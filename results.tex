\chapter{Results}                                                                          
	...
	\subsection{Difficult vs. Easy Pattern Matching}\label{sec:r_pc_ed}
		...
		\centering
		\vspace{-1.8cm}
		\scalebox{0.6}{\includegraphics{./img/cell10_cell22.png}}
	    ...
	    A graphical depiction of this cluster is presented in figure~\ref{fig:r_pc_ed}.
	    The main cluster peak is assigned the “superior parietal lobule - left” ICBM label by the WFU PickAtlas. 
	    \py{tex_mr([["A",22,1.000,0.988,4.43,-39,-58,58],["n","n",1.000,0.988,3.52,-42,-49,58]],caption="Summary statistics for brain activation clusters in the contrast of difficult pattern matching against easy pattern matching tasks. FWE = Family-Wise Error; FDR = False Discovery Rate; X, Y, and Z coordinates given in MNI space.",label="tab:r_pc_ed")}
	    
	    The inverse contrast yielded no activation clusters at our preferred significance level.
	\subsection{Emotion vs. Pattern Recognition}\label{sec:r_pc_ep}
	    ...
	    For the subsequently presented data we define pattern recognition as the baseline, and view activation prompted by emotion recognition as relative to that baseline.
		...
		These activation clusters lie predominantly in the occipital and superior parietal lobules and are accurately documented by table~\ref{tab:r_pc_ep_41i}.
	...
	\subsection{Strong vs. Weak emotion Intensity}\label{sec:r_pc_sw}
	    ...
	    Comparing the emotion matching trials based on difficulty - difficult matching against a baseline of easy matching, shows no significant brain activation at our preferred significance and spatial extent threshold.
	    ...
	    \py{tex_mr([["A",26,1.000,0.981,4.06,45,20,25],["n","n",1.000,0.981,3.17,42,5,28]],caption="Summary statistics for brain activation clusters in the contrast of difficult emotion matching tasks against easy emotion matching tasks. FWE = Family-Wise Error; FDR = False Discovery Rate; X, Y, and Z coordinates given in MNI space.",label="tab:r_pc_swi")}
	    
	    The inverse contrast of the aforementioned comparison - easy emotion recognition against a baseline of difficult emotion recognition - is also the contrast of strongly emotional stimuli against a baseline of faintly emotional stimuli.
